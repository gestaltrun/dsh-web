...
const GOOD = JSON.stringify(
  {
    ...
    private: true,
    dsh: { profile: { bundles: ['@deepseek-ai/dsh-base', '@gestaltrun/dsh-web-all'] } },
    dependencies: { '@gestaltrun/dsh-web-all': '0.2.7' },
  },
  null,
  2,
) + '\n'
...
describe('parseProfileManifest', () => {
  it('parses a valid manifest', () => {
    ...
    expect(facts.bundles).toEqual(['@deepseek-ai/dsh-base', '@gestaltrun/dsh-web-all'])
    expect(facts.dependencies).toEqual({ '@gestaltrun/dsh-web-all': '0.2.7' })
...
describe('readProfileManifest', () => {
  it('reads through the injected fs', async () => {
    ...
    expect(result.facts.bundles).toContain('@gestaltrun/dsh-web-all')
  })
...
describe('editManifestJson', () => {
  it('sets and removes by dotted path, preserving other fields', () => {
    ...
    expect(parsed.dependencies['@gestaltrun/dsh-web-all']).toBe('0.2.7')
  })

  it('reports no change when the value already matches', () => {
    const edited = editManifestJson(GOOD, { set: { 'dsh.profile.bundles': ['@deepseek-ai/dsh-base', '@gestaltrun/dsh-web-all'] } })
