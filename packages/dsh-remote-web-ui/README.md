...
- **Security**: one active token at a time (a refresh invalidates the old link; the link stays re-usable within its expiry window so a scan handed between browsers can complete pairing; it expires). Stop revokes every paired device and the current token — the `/remote` channel closes their active HTTP streams and WebSockets and rejects subsequent requests. Pairing is this plugin's access control for the `/remote` channel; direct `/api` on a LAN-exposed bind is governed by the harness fence + browser auth (see Security model). Loopback (127.0.0.1) keeps using `/api` directly. A paired device is a **full-control credential** (see Security model).
...
The Gestalt Desktop Host keeps its private `dsh-app://app` transport and opens a separate remote listener only while this plugin is enabled. It initially binds `127.0.0.1` on an available port (`desktopPort` selects a fixed startup port). Use the native remote-access settings card to enable LAN access; the listener immediately rebinds to `0.0.0.0` on the same port. Disabling LAN access restores loopback. Desktop does not edit the Web profile or manage firewall rules. No public tunnel starts by default.

Create a QR from the native sidebar entry and open it on the other device. The paired browser loads the official Web interface without Desktop's private stream scripts. Desktop always requires device pairing for the `/remote` channel, regardless of `requirePairingForLan`; real network clients cannot bypass it through direct `/api` requests. Native settings and pairing controls continue through the private transport. Stop and per-device revoke also close already-open remote HTTP streams and WebSockets.
...
4. **To pair a PC instead**: copy the same link and open it in a browser on the other computer. After the same round trip the full Web GUI runs there over the gated `/remote` channel; unpaired PCs see the guided blocking page and no data. The active link can pair multiple devices until it expires; refreshing the QR invalidates that link.
5. The desktop badge flips to 已连接 in real time; the device roster lists the paired devices with per-device 取消配对, and Stop revokes everything.
